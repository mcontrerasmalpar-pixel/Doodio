  ...
  return (
    <div style={{ width:"100%", height:"100%", display:"flex", flexDirection:"column", overflow:"hidden", background:"#5BC8F5", fontFamily:"'Chewy', cursive" }}>
      {showSaveModal && drawingDataUrl && (
        <SavePetModal drawingDataUrl={drawingDataUrl} ownerName={ownerName} onSave={handleSavePet} onClose={() => setShowSaveModal(false)} saving={saving} />
      )}
      <header style={{ flexShrink:0, background:"#FFE033", borderBottom:"3px solid #1A1A1A", boxShadow:"0 3px 0 #1A1A1A", padding:"6px 12px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"8px", zIndex:10 }}>
        ...
        <div style={{ display:"flex", gap:"5px", flexWrap:"wrap" }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => !t.locked && goTo(t.id)} style={{ padding:"5px 10px", borderRadius:"50px", background: screen===t.id ? "#FF8C42" : t.locked ? "#E8E0C8" : "#FFFBF2", border: screen===t.id ? "3px solid #1A1A1A" : "2px solid #1A1A1A", color: t.locked ? "#AAA" : "#1A1A1A", cursor: t.locked ? "not-allowed" : "pointer", fontFamily:"'Chewy'", fontSize:"0.78rem", boxShadow:"2px 2px 0 #1A1A1A", transform: screen===t.id ? "translate(1px,1px)" : "none", whiteSpace:"nowrap", touchAction:"manipulation" }}>
              {t.label}{t.locked ? " 🔒" : ""}
            </button>
          ))}
        </div>
        <button onClick={() => drawingDataUrl && setShowSaveModal(true)} style={{ display:"flex", alignItems:"center", gap:"6px", background: savedPet ? "#B8E04A" : "#FFFBF2", border:"3px solid #1A1A1A", borderRadius:"50px", padding:"4px 12px", boxShadow:"3px 3px 0 #1A1A1A", cursor: drawingDataUrl ? "pointer" : "default", fontFamily:"'Chewy'", flexShrink:0, touchAction:"manipulation" }}>
          {drawingDataUrl ? <img src={drawingDataUrl} style={{ width:"22px",height:"22px",borderRadius:"4px",border:"2px solid #1A1A1A",objectFit:"cover" }} /> : <span style={{ fontSize:"0.95rem" }}>🎵</span>}
          <span style={{ fontSize:"0.8rem", color:"#1A1A1A" }}>{savedPet ? `${savedPet.name} ✅` : "Save"}</span>
        </button>
      </header>
      <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
        ...
        {screen === "play"       && <PlayMode drawingDataUrl={drawingDataUrl} onMelodyReady={setMelody} onPlayingChange={setIsPlaying} externalPlayTrigger={playTrigger} onSavePet={() => setShowSaveModal(true)} savedPet={savedPet} onStopRef={stopMelodyRef} />}
        {screen === "experiment" && <ExperimentMode melody={melody} />}
        {screen === "profile"    && <PetProfile currentPet={savedPet} melody={melody} onPlayMelody={() => { goTo("play"); setPlayTrigger(p => p+1); }} isPlaying={isPlaying} />}
      </div>
      <div style={{ flexShrink:0, height:"28px", background:"#B8E04A", borderTop:"3px solid #1A1A1A", display:"flex", alignItems:"center", justifyContent:"center", gap:"10px" }}>
